...
const SpecialMenuSelectionPage = () => {
  ...
  const selectedDishes = menuItemsData.filter((item) =>
    ...
    <div className="w-full min-h-screen flex flex-col items-center justify-between px-4 py-8 animate-fade-in text-white">
      <div className="w-full max-w-6xl bg-black/60 backdrop-blur-md border border-white/10 rounded-3xl p-6 md:p-10 shadow-2xl">
        <div className="text-center space-y-2 mb-8">
          ...
          <HeroHeading
            heading="Select Special Dishes"
            paragraph="Your booking is qualified for pre-ordered chef specials. Select one or multiple items to complement your table reservation."
          />
        </div>
        ...
        {specialDishes.length > 0 ? (
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-12 text-center max-w-md mx-auto space-y-3 mb-10">
            ...
            <p className="text-xs text-gray-400">
              No special items match your search. Clear your search query to
        ...
        <div className="border-t border-white/10 pt-6 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="text-left text-xs text-gray-300">
            <p className="font-semibold text-white text-sm">
              Reservation:{' '}
              <span className="text-[#c93400]">{bookingDetails?.date}</span> at{' '}
              <span className="text-[#c93400]">{bookingDetails?.time}</span> (
              {bookingDetails?.guests} guests)
            </p>
            <p className="text-gray-400 mt-1">
              Selected Special Items:{' '}
              {selectedDishes.length > 0
                ? selectedDishes.map((d) => d.title).join(', ')
                : 'None'}
            </p>
          </div>
          ...
          <div className="flex gap-4 w-full md:w-auto">
            ...
            >
              {selectedDishes.length > 0
                ? `Confirm (${selectedDishes.length}) Items & Verify`
                : 'Skip Specials & Verify Table'}
            </button>
          </div>
        </div>
      </div>
